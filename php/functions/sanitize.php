...
function setArray($path,&$array,$value=null){
    $path = explode('/',$path);
        $currArr = &$array;
...
function delArray($path,&$array){
    $pathArr = explode('/',$path);
    $currArr = &$array;

    for($i=0,$lenI=count($pathArr);$i<$lenI;$i++){
        if(isset($currArr[$pathArr[$i]])){
            if($i==$lenI-1){
                unset($currArr[$pathArr[$i]]);
                return true;
            }else{
                $currArr = &$currArr[$pathArr[$i]];                
            }
