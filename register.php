	...
	if(Token::check_PerForm('register.php',Input::get(Config::get('session/token_name')))){
		...
		if($validate->passed()){
			...
			try{
				...
				Session::flash('indexdotphp',"Hello ".Input::get('name').",your account are ready to use!");
...
<form action="" method="post">
	<div class="field">
		...
		<input type="text" name="user_id" id="username" value="<?php echo Input::toHtml('user_id'); ?>" autocomplete="off">
	</div>
	<div class="field">
	...
	<div class="field">
		...
		<input type="text" name="name" id="realname" value="<?php echo Input::toHtml('name'); ?>" autocomplete="off">
	</div>
	<div>
		<input type="hidden" name="<?php echo Config::toHtml('session/token_name') ?>" value="<?php echo Token::gen_PerForm('register.php'); ?>" >
	</div>
	<input type=submit>
</form>
